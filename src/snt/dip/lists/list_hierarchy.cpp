...
namespace snt::dip {
    ...
    void HierarchyList::record(const BaseNode::PointerType& node, const std::vector<NodeDtype>& excluded) {
        ...
        while (parents.size() > 0 and node->indent <= parents.back().indent)
            parents.pop_back();
        parents.push_back({node->indent, node->name, node->collections});

        // aggregate all path collections
        std::vector<CollectionAccess> collections_full;
        ...
        if (collections_full.empty())
            throw std::runtime_error("Node does not belong to any collection: " + node->name);
        ...
        for (; it != std::prev(collections_full.end()); ++it) {
            ...
            if (cnode.type == CollectionType::MAP) {
            ...
            } else if (cnode.type == CollectionType::LIST) {
                if (itc == collections.end())
                    throw std::runtime_error("Collection does not exist: " + name_full);
        ...
        it = std::prev(collections_full.end());
        {
            ...
            if (cnode.type == CollectionType::MAP) {
                if (it == collections.end()) { // create new collection
                    collections[name_full] = Collection(name_full, {cnode.item}, CollectionType::MAP);
                } else if (it->second.type != CollectionType::MAP) {
                ...
                if (it == collections.end()) { // create new collection
                    key = "0";
                    collections[name_full] = Collection(name_full, {key}, CollectionType::LIST);
                } else if (it->second.type != CollectionType::LIST) {
        ...
        node->collections = std::move(collections_full);
    }
