    ...
    std::string Quantity::info() const {
        ...
        for (const auto& quant : puq::UnitSystem::current.data->QuantityList) {
            if (puq::BaseUnits(quant.second.definition).dimensions() != dim)
                continue;
            quantities.push_back(quant.first);
        }
        if (!quantities.empty()) {
            ss << '\n';
            ss << "Quantities:  ";
