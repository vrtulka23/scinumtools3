#ifndef SNT_MAT_EXCEPTIONS_H
#define SNT_MAT_EXCEPTIONS_H

#include <snt/core/exceptions.h>

namespace snt::mat {

    class Exception : public core::Exception {
      public:
        explicit Exception(core::ExceptionInfo info) : core::Exception(std::move(info), "[SNT-MAT] ") {}
        explicit Exception(
            std::string message,
            std::string details,
            std::string suggestion,
            std::string origin_file,
            size_t origin_line
        )
            : mat::Exception(
                  core::ExceptionInfo{
                      std::move(message),
                      std::move(details),
                      std::move(suggestion),
                      core::SourceLocation{std::move(origin_file), origin_line}
                  }
              ) {}
    };

    /**
     * Parser exceptions occurs when internal MAT parsers fails
     */
    class ParserException : public mat::Exception {
      public:
        using mat::Exception::Exception;
    };

    /**
     * Syntax exceptions are triggered by incorrect syntax
     */
    class SyntaxException : public mat::Exception {
      public:
        using mat::Exception::Exception;
    };

    /**
     * Missing exception is triggered when functionality is not implemented yet
     */
    class MissingException : public mat::Exception {
      public:
        MissingException(std::string message, std::string file, std::size_t line)
            : mat::Exception(
                  core::ExceptionInfo{
                      std::move(message),
                      "",
                      "If you need this functionality, please contact the developers, open an issue or send a pull "
                      "request on GitHub.",
                      core::SourceLocation{file, line}
                  }
              ) {};
    };

} // namespace snt::mat

#endif // SNT_MAT_EXCEPTIONS_H
