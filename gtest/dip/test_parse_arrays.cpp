...
TEST(ParseArrays, BooleanValue) {
  ...
  EXPECT_EQ(node->value_raw, dip::Array::StringType({"true","false","true","true","true","false"}));
  EXPECT_EQ(node->value_shape, dip::Array::ShapeType({2,3}));
  EXPECT_EQ(node->dtype, dip::NodeDtype::Boolean);
  EXPECT_EQ(node->indent, 0);
  EXPECT_EQ(node->name, "foo");
  EXPECT_EQ(node->dimension, dip::Array::RangeType({{2,2},{3,3}}));
...
TEST(ParseArrays, IntegerValue) {
  ...
  dip::BaseNode::PointerType node = env.nodes.at(0);
  EXPECT_EQ(node->value_raw, dip::Array::StringType({"1","2","3","-4","-5","-678910111"}));
  EXPECT_EQ(node->value_shape, dip::Array::ShapeType({2,3}));
...
TEST(ParseArrays, FloatValue) {
  ...
  dip::BaseNode::PointerType node = env.nodes.at(0);
  EXPECT_EQ(node->value_raw, dip::Array::StringType({"1","2.2","3.3e3","-4","-5.5","-6.6e6"}));
  EXPECT_EQ(node->value_shape, dip::Array::ShapeType({2,3}));
...
TEST(ParseArrays, StringValue) {
  ...
  dip::BaseNode::PointerType node = env.nodes.at(0);
  EXPECT_EQ(node->value_raw, dip::Array::StringType({"position","velo,ci\"ty","acce]lera'tion","jerk","snap","crackle"}));
  EXPECT_EQ(node->value_shape, dip::Array::ShapeType({2,3}));
